...
class MobileSignature extends Component {
  ...
  handleChange = (event) => {
    console.log(event.target.innerValue)
  }
  render(){
    ...
    let fontStyle = 'margin:0;font-size:15px;font-family:"Calibri",sans-serif';
    ...
    if (isaText !== '') {
      isaLine = (
        `<p style=${fontStyle}><span style="fontSize: '11px'">ISA Certified Arborist ${isaText}</span></p>`)
    }
    ...
    if (additionalText !== '') {
      additionalLine = (
        `<p style=${fontStyle}><span style="fontSize: '11px'">${additionalText}</span></p>`
      )
    }
      ...
      let htmlSignature = `${first}${second}${isaLine}${additionalLine}${logo}${statement}${icons}`;
      
      return (
        <div>
        <CopyToClipboard text={htmlSignature}
          onCopy={() => this.setState({copied: true})}>
          <button onClick={this.handleClick}>Copy HTML for iOS</button>
        </CopyToClipboard>
          <div 
