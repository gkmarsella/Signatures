    ...
    let htmlSignature = `${first}${second}${third}${fourth}${isaLine}${additionalLine}${logo}${statement}${icons}${disclosure}`;
      ...
      return (
        ...
        <CopyToClipboard text={htmlSignature}
          ...
          <button type="button" className={ `btn instruction-button ${this.props.isEnabled ? null : 'disabled'}`} tabIndex={this.props.isEnabled ? 0 : -1} onClick={this.handleClickShowAlert.bind(this)} data-container="body" data-toggle="popover" data-placement="top" data-content="Copied to clipboard!">
          <img className="btn-icon" src={Outlook_Icon} alt="Outlook Icon" /><strong> Copy signature for iOS Outlook application</strong></button>
        </CopyToClipboard>
